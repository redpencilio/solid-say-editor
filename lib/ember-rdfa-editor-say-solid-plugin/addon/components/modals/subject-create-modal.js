...
export default class SubjectCreateModalComponent extends ModalComponent {
    ...
    async save(){
        ...
        for (let [_, form] of Object.entries(this.forms)) {
            ...
            let statement = new Statement(rdflib.sym(this.args.graph.value + "#" + subjectObj.subject), RDF("type"), rdflib.sym(subjectObj.type.vocab + subjectObj.type.value), this.args.graph);
            if (statement) {
    ...
    addForm() {
        ...
        form.model = {subject: "", type: "", typeVocab: ""};
