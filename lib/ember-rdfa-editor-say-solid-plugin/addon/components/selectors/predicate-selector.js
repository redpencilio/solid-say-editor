...
export default class PredicateSelectorComponent extends Component {
  ...
  async resetAttributes() {
    ...
    for (const quad of quads) {
      ...
      let predicateObj = new Predicate(quad.predicate, metadata[RDFS("label").value], metadata[RDFS("comment").value], quad.object);
  ...
  async addPredicate(predicate, object){
    ...
    let predicateObjs = new Set(this.attributes);
    predicateObjs.add(predicateObj);
    this.attributes = predicateObjs;
  }
}