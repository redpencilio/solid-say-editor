   ...
   */
    fromRDFa(block, triples, editor) {
        triples.forEach(triple => {
            ...
            const originalObjectValue = this.extractOriginalObjectValue(block);
    ...
    extractOriginalObjectValue(block){
        if(block.richNode.length){
            let attributes = block.richNode[0].parent.domNode.attributes;
            console.log(attributes["data-original"]);
            return attributes["data-original"] ? attributes["data-original"].value.trim() : undefined;
        }
    }
