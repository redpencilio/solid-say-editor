...
class RDFaSerializer extends ModelSerializer {
    ...
    _serializePredicateObject({predicate, object}, metadata){
        ...
        if (object.termType === "Literal") {
            return `<span property="${predicate.value}" content="${object.value}" title="${comment}"> ${object.value} </span>`;
        } else {
            return `<a property="${predicate.value}" href="${object.value}" title="${comment}">${object.value}</a>`;
        }
    }
