...
export default class RdfaCommunicatorService extends Service {
    ...
    getGraphByType(type, typeIndex){
        ...
        if(type && typeIndex){
            const match = this.store.any(undefined, SOLID("forClass"), type, typeIndex);
            if(match){
                return this.store.any(match.subject, SOLID("instance"), undefined, typeIndex).object;
            }
        }
        return false;
    }
        ...
        rdfa.forEach(triple => {
            if(triple.subject === subject.value && triple.predicate !== "a"){
                ...
                if(triple.datatype === RDFS("Resource").value){
                    ...
                    newObject = rdflib.sym(triple.object.trim());
                } else {
