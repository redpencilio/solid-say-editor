...
import { RDF, SOLID, FOAF } from 'solid-addon/utils/namespaces';
...
export default class RdfaCommunicatorService extends Service {
        ...
        rdfa.forEach(triple => {
            if (triple.predicate !== "a") {
                ...
                if (!dels || dels[0].object.value.trim() !== triple.object.trim()) {
                    ...
                    this.addTripleToCache(ins);
                    this.store.graph.add(ins)
                    if(dels){
                        this.store.graph.remove(dels[0]);
                    }
                }
            }
        });
    ...
    fetcherReset(graph) {
        return new Promise((resolve, reject) => {
            this.store.fetcher.nowOrWhenFetched(graph, { force: true, clearPreviousData: true }, (ok, body, xhr) => {
                if (!ok) {
                    reject(new Error("Could not load triples"))
                } else {
                    console.log(this.store.match(undefined, FOAF("name"), undefined, graph));
                    this.insertCache = [];
                    resolve("Done");
                }
            });
