...
export default class ModelStoreCommunicatorService extends Service {
    ...
    /**
     * Adds the given quad statement and the associated uuid to the insert cache.
     * 
     * @param {Object} quad statement to add to the cache.
     * 
     */
    addQuadToCache({ quad, objectUuid }) {
        this.insertCache = this.insertCache.filter(ins =>
            !(ins.triple.subject.equals(quad.subject) &&
                ins.triple.predicate.equals(quad.predicate) &&
                ins.triple.graph.equals(quad.graph) &&
                ins.objectUuid === objectUuid)
        );

        this.insertCache.push({ triple: quad, objectUuid });
    }
    ...
    */
    ...
    /**
     * The same functionality as `fetchTriples(..)` but returns an empty array 
     * if the request gets rejected. 
     * 
     * @param {NamedNode} subject NamedNode representing the subject to be queried in the forking store.
     * @param {Object} attributes Object containing the predicate uri strings as key and predicate NamedNode as value. 
     * @param {NamedNode} graph NamedNode representing the graph in which the subject will be queried. 
    ...
    * @returns {Object} map containing the value for the metadata about the given resources. 
    */
    async fetchResourceMetaData(resource) {

        //TODO: Store this list of metadata to be queried, in a config file. 
   ...
   */
    ...
    /**
     * Updates the attribute of specified in the `oldQuad` statement with the attribute value in 
     * the `newQuad` statement.
     * 
     * @param {Statement} oldQuad old statement which will be updated to values in `newQuad`. 
     * @param {Statement} newQuad new statemenet which will be the updated version in the store.
     * @param {String} objectUuid uuid which identifies the attribute that is being updated. 
     */
    updateQuad(oldQuad, newQuad, objectUuid) {
        ...
        if (dels.length && dels[0].object.value.trim() !== newQuad.object.value.trim()) {
            ...
            this.addQuadToCache({ quad: ins[0], objectUuid });
    ...
    /**
     * Add the given object's value into the map, which has the uuid of the attribute as key. 
     * @param {NamedNode} object NamedNode containing the value for the corresponding attribute uuid.
     * @returns {String} uuid of the attribute to which the `object`'s value is bound to. 
     */
